...
-(instancetype)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier {
    if (self = [super initWithStyle:style reuseIdentifier:reuseIdentifier]) {
        ...
        self.m_contentLabel = [LSKViewFactory initializeLableWithText:nil font:14 textColor:KColorHexadecimal(kText_Title_Color, 1.0) textAlignment:1 backgroundColor:nil];
...
-(void)setupContentWithTitle:(NSString *)title isSeleted:(BOOL)isSeleted {
    ...
    if (isSeleted) {
        self.m_contentLabel.textColor = [UIColor whiteColor];
...
-(void)changeTitleAttribute:(CGFloat)rate {
    ...
    self.m_contentLabel.textColor = KColorRGBA(ceil(32 + (223 * rate)), ceil(32 + (223 * rate)), ceil(32 + (223 * rate)), 1.0);
}
