...
-(instancetype)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier {
    if (self = [super initWithStyle:style reuseIdentifier:reuseIdentifier]) {
        ...
        self.m_contentLabel = [LSKViewFactory initializeLableWithText:nil font:WIDTH_RACE_6S(13) textColor:KColorHexadecimal(kText_Title_Color, 1.0) textAlignment:1 backgroundColor:nil];
...
-(void)setupContentWithTitle:(NSString *)title isSeleted:(BOOL)isSeleted {
    ...
    if (isSeleted) {
        self.m_contentLabel.textColor = KColorHexadecimal(0xe798f7, 1.0);
        [UIView animateWithDuration:0.2 animations:^{
            self.m_contentLabel.transform = CGAffineTransformMakeScale(1.13, 1.13);
        }];
    }else
    {
        self.m_contentLabel.textColor = KColorHexadecimal(kText_Title_Color, 1.0);
        [UIView animateWithDuration:0.2 animations:^{
...
-(void)changeTitleAttribute:(CGFloat)rate {
    ...
    self.m_contentLabel.textColor = KColorRGBA(ceil(32 + (199 * rate)), ceil(32 + (120 * rate)), ceil(32 + (215 * rate)), 1.0);
}
