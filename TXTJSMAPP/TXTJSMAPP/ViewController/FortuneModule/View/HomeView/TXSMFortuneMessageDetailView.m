...
- (void)changeProductShow:(BOOL)isTap {
    if (!isTap) {
        ...
        [UIView animateWithDuration:0.25 animations:^{
            [self.m_mainScrollView setContentOffset:CGPointMake((SCREEN_WIDTH - 20) * self->_currentIndex, 0)];
...
- (void)frameChange {
    ...
    if (contentHeight != scrollHeight) {
        self.m_mainScrollView.contentSize = CGSizeMake((SCREEN_WIDTH - 20) * (self.m_naviTitleArray.count - 1), contentHeight);
...
-(void)changeUnderLineFrame {
    ...
    [UIView animateWithDuration:0.2 animations:^{
        self.underLineView.frame = CGRectMake(10 + rect.origin.y, kHomeHeadButtonHeight - WIDTH_RACE_6S(1.5) - 10, rect.size.height, WIDTH_RACE_6S(1.5));
    }];
}
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    ...
    if (!_isClickAnimal) {
        CGFloat screenWidth = SCREEN_WIDTH - 20;
...
-(void)changeLineFrame:(CGFloat)rate currentFrame:(CGRect)currentFrame lastFrame:(CGRect)lastFrame direction:(NSInteger)direction lastRate:(CGFloat)lastRate{
    ...
    lastFrame.origin.y = lastFrame.origin.y + 10;
    if (rate != 1) {
        CGFloat lineWidth = 0;
        if (direction) {
            ...
            self.underLineView.frame = CGRectMake(x, kHomeHeadButtonHeight - WIDTH_RACE_6S(1.5) - 10, lineWidth , WIDTH_RACE_6S(1.5));
        }else
        {
            ...
            self.underLineView.frame = CGRectMake(lastFrame.origin.y - x , kHomeHeadButtonHeight - WIDTH_RACE_6S(1.5) - 10, lineWidth , WIDTH_RACE_6S(1.5));
        }
    }else
    {
        self.underLineView.frame = CGRectMake(lastFrame.origin.y, kHomeHeadButtonHeight - WIDTH_RACE_6S(1.5) - 10, lastFrame.size.height, WIDTH_RACE_6S(1.5));
...
- (CGFloat)widthListView:(HListView *)listView OfColumnForIndex:(NSInteger)index {
    CGFloat width = (SCREEN_WIDTH - 20) / self.m_naviTitleArray.count;
    return width;
}
...
- (void)_layoutMainView {
    _currentIndex = 0;
    UIView *bgView = [[UIView alloc]initWithFrame:CGRectMake(10, kHomeHeadButtonHeight - 10, SCREEN_WIDTH - 20, 20)];
    bgView.backgroundColor = [UIColor whiteColor];
    [self addSubview:bgView];
    self.m_naviTitleArray = [[NSArray alloc]initWithObjects:@"今日运势",@"明日运势",@"本周运势",@"本月运势",@"今年运势", nil];
    _currentIndex = 0;
    [self customHeadButtonView];
    self.m_mainScrollView = [[UIScrollView alloc]initWithFrame:CGRectMake(10, kHomeHeadButtonHeight, SCREEN_WIDTH - 20,0 )];
    ...
    for (int i = 0; i < self.m_naviTitleArray.count - 1; i++) {
        if (i < 2) {
            ...
            view.frame = CGRectMake((SCREEN_WIDTH - 20) * i, 0, (SCREEN_WIDTH - 20), [view returnViewHeight]);
...
- (void)customHeadButtonView {
    self.m_naviListView = [[HListView alloc]initWithFrame:CGRectMake(10, 0, SCREEN_WIDTH - 20, kHomeHeadButtonHeight)];
    KViewBoundsRadius(self.m_naviListView, 10);
    ...
    self.underLineView = [[UIView alloc]initWithFrame:CGRectMake(10 + WIDTH_RACE_6S(9), kHomeHeadButtonHeight - WIDTH_RACE_6S(1.5) - 10, WIDTH_RACE_6S(53.5), WIDTH_RACE_6S(1.5))];
