...
- (void)setupContent:(NSDictionary *)dict messageDict:(NSDictionary *)message {
    ...
    self.messageView.frame = CGRectMake(10, 100, SCREEN_WIDTH - 20, height);
    _topMessageHeight = 100 + height + 5;
...
- (void)_layoutMainView {
    UIImageView *bgImage = [[UIImageView alloc]initWithImage:[ImageNameInit(@"bgViewimage")resizableImageWithCapInsets:UIEdgeInsetsMake(558, 10, 0, 10) resizingMode:UIImageResizingModeStretch]];
...
- (TXSMFortuneMessageView *)messageView {
    if (!_messageView) {
        TXSMFortuneMessageView *messageView = [[TXSMFortuneMessageView alloc]initWithFrame:CGRectMake(10, 100, SCREEN_WIDTH - 20, 0)];
