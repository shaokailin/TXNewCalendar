...
- (void)setupLastImageBtnView:(UIImageView *)imageView type:(NSInteger)type {
    ...
    UIButton *startBtn = [LSKViewFactory initializeButtonWithTitle:@"立即体验" nornalImage:nil selectedImage:nil target:self action:@selector(startBtnClick) textfont:15 textColor:KColorHexadecimal(0x6c6c6c, 1.0) backgroundColor:nil backgroundImage:nil];
    KViewRadius(startBtn, 1);
    KViewBorderLayer(startBtn, KColorHexadecimal(0xefc091, 1.0), kLineView_Height);
    CGFloat height = 0;
    if (type == 0) {
        height = CGRectGetHeight(imageView.frame) - WIDTH_RACE_6S(90);
    }else if (type == 1) {
        height = CGRectGetHeight(imageView.frame) - 70;
    }else {
        height = CGRectGetHeight(imageView.frame) - 90;
    }
    
    startBtn.frame = CGRectMake((SCREEN_WIDTH - WIDTH_RACE_6S(145)) / 2.0 , height,  WIDTH_RACE_6S(145),  WIDTH_RACE_6S(29));
