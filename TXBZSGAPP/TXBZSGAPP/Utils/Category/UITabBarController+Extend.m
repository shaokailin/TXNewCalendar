...
- (UINavigationController *)initializeNavigationWithClass :(NSString *)className
    ...
    [self setupTabBarItemWithTabBarItem:navigation.tabBarItem title:tabbarTitle nornalImage:KJudgeIsNullData(nornalImage) ? [UIImage imageNamed:nornalImage] : nil seletedImage:KJudgeIsNullData(seletedImage) ? [UIImage imageNamed:seletedImage] : nil];
    return navigation;
}
...
- (void)setupTabBarBackgroundColor:(UIColor *)bgColor {
    ...
    [self.tabBar setShadowImage:[LSKImageManager imageWithColor:KColorHexadecimal(kMainBackground_Color, 1.0) size:CGSizeMake(SCREEN_WIDTH, 0.5)]];
}
...
//        [item setImageInsets:UIEdgeInsetsMake(-5, 0.0, 0, 0.0)];
//    }
}
@end
