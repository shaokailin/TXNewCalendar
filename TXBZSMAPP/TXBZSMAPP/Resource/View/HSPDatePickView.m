...
- (void)initializeNaviButtonView {
    ...
    lineView.backgroundColor = KColorHexadecimal(kLineMain_Color, 1.0);
    ...
    lineView1.backgroundColor = KColorHexadecimal(kLineMain_Color, 1.0);
    ...
    UIButton *cancleBtn = [LSKViewFactory initializeButtonWithTitle:@"取消" nornalImage:nil selectedImage:nil target:self action:@selector(cancleSelectedClick) textfont:15 textColor:KColorHexadecimal(0x333333, 1.0) backgroundColor:nil backgroundImage:nil];
    ...
    UIButton *sureBtn = [LSKViewFactory initializeButtonWithTitle:@"确定" nornalImage:nil selectedImage:nil target:self action:@selector(sureSelectClick) textfont:15 textColor:KColorHexadecimal(0x333333, 1.0) backgroundColor:nil backgroundImage:nil];
...
- (void)showInView {
    if (!isShow) {
        ...
        [UIView animateWithDuration:0.25 animations:^{
            ...
            self->_contentView.frame = CGRectMake(0, CGRectGetHeight(self.frame) - 220 - self->_tabbarHeight , SCREEN_WIDTH, 220);
        }];
    }
}
...
- (void)cancleSelectedClick {
    [UIView animateWithDuration:0.25 animations:^{
        ...
        self->_contentView.frame = CGRectMake(0, CGRectGetHeight(self.frame), SCREEN_WIDTH, 220);
