...
-(instancetype)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier {
    if (self = [super initWithStyle:style reuseIdentifier:reuseIdentifier]) {
        ...
        self.m_contentLabel = [LSKViewFactory initializeLableWithText:nil font:WIDTH_RACE_6S(15) textColor:[UIColor blackColor] textAlignment:1 backgroundColor:nil];
...
-(void)setupContentWithTitle:(NSString *)title isSeleted:(BOOL)isSeleted {
    ...
    if (isSeleted) {
        self.m_contentLabel.textColor = KColorHexadecimal(0xdd33ee, 1.0);
        [UIView animateWithDuration:0.2 animations:^{
            self.m_contentLabel.transform = CGAffineTransformMakeScale(1.13, 1.13);
        }];
    }else
    {
        self.m_contentLabel.textColor = KColorHexadecimal(0x222222, 1.0);
        [UIView animateWithDuration:0.2 animations:^{
...
-(void)changeTitleAttribute:(CGFloat)rate {
    ...
    self.m_contentLabel.textColor = KColorRGBA(ceil(34 + (187 * rate)), ceil(34 + (17 * rate)), ceil(34 + (204 * rate)), 1.0);
}
